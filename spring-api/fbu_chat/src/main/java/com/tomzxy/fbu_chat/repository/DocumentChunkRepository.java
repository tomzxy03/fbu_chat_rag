...
public interface DocumentChunkRepository extends JpaRepository<DocumentChunk, UUID> {
     ...
     * Chỉ trả về chunks có cosine distance < threshold (tức similarity > 1 - threshold).
    ...
    /**
     * Vector search với filter tùy chọn theo year và/hoặc docType + similarity threshold.
     */
    @Query(value = """
            SELECT content, source_file AS sourceFile, year, doc_type AS docType
            FROM document_chunks
     ...
     * - Full-text search: tìm theo từ khóa chính xác (số điều, mã văn bản, tên riêng)
     *
